...
def run_transdecoder_blastp(transcripts,num_cores,strand,DIR,blastp_db):
	"""translate"""
	...
	if fasta_ok(DIR+outpep) and fasta_ok(DIR+outcds): print "Skip transdecoder"
		...
		if blastpout_ok(DIR+blastpout): print "Skip blastp"
			...
			cmd = "blastp -query "+(DIR+allpep)+" -db "+"blastp_db
...
def transrate_cds_ref(outcds,num_cores,reference="/dlocal/home/2019013/Data/Colab/Transcriptome_Assembly_Phyllostomids/scripts/database-blast/Hg38_Ensembl111_proteome_n23214_27022024_ENSG-ENST.faa"):
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 6:
		...
		transcripts,num_cores,strand,DIR,blastp_db = sys.argv[1:]
		...
		run_transdecoder_blastp(transcripts,num_cores,strand,DIR,blastp_db)
		check_pep_coverage_redundancy(blastpout)
		#transrate_cds_ref(outcds,num_cores)

	else:
		print ("Usage:")
		print ("python transdecoder_wrapper.py transcripts num_cores strand(stranded or non-stranded) output_dir blastp_db_prefix")
		sys.exit(0)
